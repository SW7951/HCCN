...
def main(opt):
    ...
    print('Acc: ')
    print(' & '.join([f'{x:.4f}' for x in result.compute()[0]]) + f' & {result.compute()[1]:.4f}')
    print('AUC:')
    print(' & '.join([f'{x:.4f}' for x in auc]) + f' & {torch.mean(auc):.4f}')
    print('PR-AUC:')
    print(' & '.join([f'{x:.4f}' for x in pr_auc]) + f' & {torch.mean(pr_auc):.4f}')
